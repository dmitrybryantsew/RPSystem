...
public partial class RpSystemViewModel : ObservableObject
{
    ...
    private void ApplyRelationshipRulesToCharacter(Character character, RpWorldContextCharacter profile)
    {
        foreach (var rule in profile.RelationshipRules ?? [])
        {
    ...
    private void ApplyRelationshipRulesToCharacter(Character character, RpFactionProfile? faction)
    {
        if (faction == null)
        {
            return;
        ...
        foreach (var rule in faction.RelationshipRules ?? [])
        {
            foreach (var matchedCharacter in World.Characters.Values.Where(candidate => RelationshipRuleMatchesCharacter(rule, candidate)))
            {
                character.KnownCharacters.Add(new Relationship
    ...
    private RpSpeciesTemplate? FindSpeciesTemplateForProfile(RpWorldContextCharacter profile)
        => (SelectedWorldContext?.SpeciesTemplates ?? [])
            .FirstOrDefault(template => SpeciesTemplateMatchesProfile(template, profile));

    private RpFactionProfile? FindFactionProfileForProfile(RpWorldContextCharacter profile)
        => (SelectedWorldContext?.Factions ?? [])
            .FirstOrDefault(faction => faction.IsEnabled && FactionProfileMatchesProfile(faction, profile));

    private static bool FactionProfileMatchesProfile(RpFactionProfile faction, RpWorldContextCharacter profile)
    {
    ...
    private static bool SpeciesTemplateMatchesProfile(RpSpeciesTemplate template, RpWorldContextCharacter profile)
    {
    ...
    private static void ApplySpeciesTemplateToCharacter(Character character, RpSpeciesTemplate? template)
    {
        ...
        {
            foreach (var part in character.Body.Where(candidate => BodyPartMatchesModifier(candidate, modifier.Key)))
            {
                if (!part.Functions.Contains(modifier.Value, StringComparer.OrdinalIgnoreCase))
                {
                    part.Functions.Add(modifier.Value);
                }
            }
        }
    }
    ...
    private static List<RpAbility> BuildProfileAbilities(RpWorldContextCharacter profile, RpFactionProfile? faction)
    {
    ...
    private static bool FactionRoleMatchesProfile(RpFactionRole role, RpWorldContextCharacter profile)
    {
    ...
    private static bool FactionAbilityMatchesProfile(RpAbility ability, RpWorldContextCharacter profile)
    {
        ...
        return roleTags.Any(roleTag =>
            ...
            profileTags.Any(tag => string.Equals(tag, roleTag, StringComparison.OrdinalIgnoreCase)));
    }

    private static void ApplyStatModifier(Character character, string key, string value)
    {
